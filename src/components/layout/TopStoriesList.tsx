            ...
            <article key={post._id} className="p-3 sm:p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  {post.slug?.current ? (
                    <Link href={`/${post.slug.current}`}>
                      <h3 className="font-serif font-bold text-vpn-gray dark:text-white text-sm sm:text-base leading-tight mb-1 hover:text-vpn-blue dark:hover:text-blue-400">
                        {post.title || "Untitled Article"}
                      </h3>
                    </Link>
                  ) : (
                    <h3 className="font-serif font-bold text-vpn-gray dark:text-white text-sm sm:text-base leading-tight mb-1">
                      {post.title || "Untitled Article"}
                    </h3>
                  )}
