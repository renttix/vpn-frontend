  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
      <div className="mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">
        <h2 className="font-roboto font-bold text-vpn-blue dark:text-yellow-500 text-xl uppercase tracking-wider" style={{ fontFamily: 'Roboto, sans-serif' }}>Legal Commentary</h2>
        ...
        {legalCommentaryPosts.length > 0 ? (
          legalCommentaryPosts.map((post) => (
            <li key={post._id} className="border-b border-gray-100 dark:border-gray-700 pb-3 last:border-0">
              <Link href={`/${post.slug?.current || `post-${post._id}`}`} className="group block">
                <div>
                  <h3 className="font-roboto font-bold text-sm text-vpn-gray dark:text-gray-100 group-hover:text-vpn-blue dark:group-hover:text-yellow-500 transition-colors duration-200" style={{ fontFamily: 'Roboto, sans-serif' }}>
                    {post.title}
                  </h3>
